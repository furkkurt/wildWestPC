class duello extends Phaser.Scene{
  ...
  create(){
    const sky = this.add.tileSprite(0,-80,5000,1000,"sky").setOrigin(0).setScale(.2).setScrollFactor(0);
    this.time.addEvent({
      delay: 25,
      callback:() =>{
        sky.x -= .1;
              ...
              callback:() =>{
                ...
                this.time.addEvent({
                  ...
                  callback:() =>{
                    ...
                    this.shoot.setVisible(true);
                    this.shoot.on("pointerdown", () => {localStorage.setItem("location", "town"); window.location.reload()});
                  }
                })
              }
            })
