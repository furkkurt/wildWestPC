class map2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.civilText = this.add.text(bulletText.x + 905, bulletText.y + 200, "0 Civilians Killed", {color: "black", fontFamily: "litebulb", fontSize:"48px"}).setDepth(999).setOrigin(0).setScrollFactor(0);
  ...
  update() {
    ...
    if (this.player.x > 115 * 70)
      this.player.x = 15 * 70;
    if (this.player.y < 15 * 70)
      this.player.y = 115 * 70;
    if (this.player.y > 115 * 70)
      this.player.y = 15 * 70;

    if (targetSprite.x < 15 * 70)
      targetSprite.x = 115 * 70;
    if (targetSprite.x > 115 * 70)
      targetSprite.x = 15 * 70;
    if (targetSprite.y < 15 * 70)
      targetSprite.y = 115 * 70;
    if (targetSprite.y > 115 * 70)
    ...
    if (mouse.isDown && gunDrawn == true && fired == false) {
      ...
      this.physics.add.collider(bullet, targetSprite, () => {
            ...
            localStorage.setItem("money", this.newMoney);
					case 9:
						rank = "S";
						break;
            this.newMoney = parseInt(parseInt(localStorage.getItem("money")) + 250);
            localStorage.setItem("money", this.newMoney);
					case 8:
						rank = "A+";
						break;
            ...
            localStorage.setItem("money", this.newMoney);
					case 5:
						rank = "B";
						break;
            this.newMoney = parseInt(parseInt(localStorage.getItem("money")) + 75);
            localStorage.setItem("money", this.newMoney);
					case 4:
						rank = "C";
						break;
            this.newMoney = parseInt(parseInt(localStorage.getItem("money")) + 50);
            localStorage.setItem("money",this.newMoney);
					case 3:
						rank = "D";
						break;
            this.newMoney = parseInt(parseInt(localStorage.getItem("money")) + 30);
            localStorage.setItem("money",this.newMoney);
					case 2:
						rank = "E";
						break;
      ...
      bullets = bullets - 1;
      bulletText.destroy();
      bulletText = this.add.text(-120, -60, "Bullets: " + bullets, {color: "black", fontFamily: "litebulb", fontSize: "32px"}).setDepth(100).setScrollFactor(0);
      if (bullets >= 1) {
